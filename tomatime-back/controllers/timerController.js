    ...
    try {
      ...
      if (lastTimer[0].state == "RUNNING") {
        ...
        const timeDifference =
          Math.abs(lastTimerDate.getTime() - new Date().getTime()) / 1000;
        ...
        if (lastTimer[0].state == "BROKEN")
          return res.status(400).json({ msg: "Il pomodoro è rotto" });
...
exports.startTimer = async (req, res) => {
  ...
  if (user_id && duration && step && description) {
    try {
      const [lastTimer] = await databasePool.query(
        "SELECT * FROM Timer WHERE id=(SELECT MAX(id) FROM Timer WHERE user_id = ?)",
        [user_id]
      );
      if (lastTimer.length > 0)
        if (lastTimer[0].state == "RUNNING")
          return res.status(200).json({ msg: "Pomodoro già avviato" });
    ...
    try {
      ...
      if (lastTimer.state) {
        if (lastTimer.state == "RUNNING") {
          ...
          return res
            .status(200)
            .json({ msg: "Il pomodoro non è ancora finito" });
        }
...
exports.stopTimer = async (req, res) => {
  ...
  if (id) {
    try {
      if (lastTimer[0].state == "RUNNING") {
        const [timer] = await databasePool.execute(
          `UPDATE Timer SET state = "BROKEN" WHERE id = ?`,
          [lastTimer[0].id]
        );
        return res
          .status(200)
          .json({ msg: "Pomodoro rotto con successo!", timer });
      }
